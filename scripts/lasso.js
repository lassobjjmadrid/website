...
$(document).ready(function() {
    ...
    // Smooth scrolling for navigation links (exclude Bootstrap accordion and modal triggers)
    $('a[href*="#"]:not([href="#"]):not([data-toggle]):not([data-parent])').click(function(e) {
        var href = $(this).attr('href');
        var hash = this.hash;
        
        // Skip if it's an external link or doesn't have a hash
        if (!hash || location.pathname.replace(/^\//, '') !== this.pathname.replace(/^\//, '') || location.hostname !== this.hostname) {
            return true;
        }
        
        // Skip if this is an accordion or modal trigger
        if ($(this).attr('data-toggle') || $(this).attr('data-parent') || $(this).closest('.panel-group').length) {
            return true;
        }
        
        var target = $(hash);
        
        // If target doesn't exist, try finding by name attribute
        if (!target.length) {
            target = $('[name="' + hash.slice(1) + '"]');
        }
        
        // If we found a target, scroll to it
        if (target.length) {
            e.preventDefault();
        ...
        // If no target found, let the browser handle it normally
        return true;
    });
    ...
    var whatsappButton = $('<a href="https://wa.me/34647547358?text=Hola!%20Me%20gustar%C3%ADa%20informaci%C3%B3n%20sobre%20las%20clases%20de%20Jiu-Jitsu%20en%20Lasso%20BJJ%20Madrid." target="_blank" id="whatsapp-float" title="Contactar por WhatsApp">' +
